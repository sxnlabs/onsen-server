...
## Hardware safety — protect the spa first

The spa pump died while this app was being developed. There is no proven causal link, but treat this as a serious hardware-safety incident: programming must never increase risk compared with careful manual use. When in doubt, fail closed, avoid writes, and leave the spa in a state the vendor controller can safely manage.

**Safety is not absolute.** The app cannot guarantee protection against firmware bugs, relay failures, blocked water flow, dirty filters, air locks, bad sensors, or anyone bypassing the app and talking directly to `<spa-ip>:8990`. The app can only make its own behavior conservative and auditable. Do not claim that automation is "impossible" to damage the spa; say what is enforced and what remains a hardware/network risk.

**Heating requires trustworthy water temperature and circulation.** Never enable `heater` unless the latest status has a valid `current_temp` and filtration/circulation is on first. If the temperature is missing or decoded as an error, the scheduler must request `heater=False`; this safety shutoff must override manual scheduler freezes. Turning `filter` off while `heater` is on must cut `heater` first.

**Scheduling must be gentle.** The scheduler may manage only `power`, `preset_temp`, `heater`, and `filter`, and only through `Supervisor.set_field()` / `Supervisor.set_preset()`. It must not rapidly cycle pump/heater relays, bypass manual override semantics except for safety shutoff, or add new modes that repeatedly toggle outputs to approximate thermostat logic. Let the spa's own thermostat hold temperature after a bounded setpoint command.

**Automated relay changes have a cooldown.** Scheduler-driven `power`/`heater`/`filter` toggles use `Scheduler.min_automation_toggle_seconds` to avoid relay chatter. Do not remove or shorten this casually. Safety shutoff (`heater=False` on sensor error) must bypass cooldown; pump/filter starts and stops must not.

**Keep setpoints bounded and model-specific.** `preset_temp` stays within `protocol.TEMP_MIN_C/MAX_C` (20–40 °C). The Baltik UI exposes only `power`, `heater`, `filter`, and `bubbles`; do not expose hidden HTTP controls for `jets` or `sanitizer` unless the real hardware is known to support them and the safety model is revisited.

**Audit every app-initiated write.** `Supervisor.set_field()` and `Supervisor.set_preset()` write command rows to `state/commands.jsonl` in production. Keep this fail-soft and append-only so a future pump/heater incident has a usable timeline.

**No real-spa writes during tests or exploratory work.** Unit/e2e tests use `tests/fake_spa.py`. `probe.py` is read-only by design; keep diagnostics read-only unless the user explicitly asks for a controlled real-device command. Never add tests, startup probes, migrations, or health checks that mutate the real spa.

**Every new write path needs safety tests.** Before changing scheduler/client/route behavior, add or update fake-spa tests that cover: read-before-toggle idempotence, lost-ack behavior, heater-on with filter-off, filter-off with heater-on, invalid/missing temperature, out-of-range setpoints, paused automation, and manual override interaction. Run `uv run pytest -q` before declaring the app safe to use.
